...
For this part of the assignment, first ignore all missing values and
...
For this part of the assignment, first ignore all missing values and
then  calculate the total number of steps taken per interval, averaged across all days.
...
first calculate the total number of missing values in the data set (i.e. the total number of rows with NAs).
observe that NAs only appear in the `steps` column:
...
Then fill in all of the missing values in the data set by replacing each missing `steps` value with the mean number of steps for that 5-minute interval.
